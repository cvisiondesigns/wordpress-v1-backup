...
const ui = {
  plane: '#digivision_plane-canvas',
  main: '#digivision_main-canvas',
  textNodes: '[data-js=digivision_text]',
  social: '#digivision_social',
  mouse: '#digivision_mouse'
}
  ...
  splitText(){
    ui.textNodes.forEach(el => {
      ...
      el.innerHTML = value.split('').reduce((acc, cur) => {
        return acc + `<span class="digivision_letter">${ cur }</span>`
      }, '')
    })
  }
  animateText(){
    const callback = () => {
      ui.social.classList.add('digivision_active')
      ui.mouse.classList.add('digivision_active')
    }
    ui.textNodes.forEach((el, elIndex) => {
      el.classList.add('digivision_active')
      const letters = el.querySelectorAll('.digivision_letter')
      const length = Math.round(letters.length / 2) + 1
      for(let i = 0; i < length; i++) {
        const [letter1, letter2] = [letters[i], letters[letters.length - i]]
        setTimeout(() => {
          if (letter1) letter1.classList.add('digivision_active')
          if (letter2) letter2.classList.add('digivision_active')
