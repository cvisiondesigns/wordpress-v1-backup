			...
			<head>
				<xsl:choose>
					<xsl:when test="kml:kml">
						<title>Locations Sitemap - 澄視維度設計 C / Vision Design</title>
					</xsl:when>
					<xsl:otherwise>
						<title>XML Sitemap - 澄視維度設計 C / Vision Design</title>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="kml:kml">
						...
						<div id="content">
							...
							<p class="expl">
								<a href="https://cvisiondesigns.com/sitemap_index.xml">&#8592; Sitemap Index</a>							</p>
